############################################# Bermoid APP SETTINGS #############################################
import os
import pathlib 

"use development" # calling development server from bermoid.core.server @noql -> 5391 
...
SECRET_KEY = "bermoid-insecure?V(_[i=`>0IRBe_W" # replace with you own strong and secure secret key.
...
# For more information, visit: http://bermoid.vvfin.in/middlewares/core/x-frame-options
# Refer to @noql -> 5369
...
# For detailed configuration information, visit: http://bermoid.vvfin.in/template/jinja2/settings
# Additional details available at @noql -> 5370.

TEMPLATES = [
    {
        "BACKEND": "bermoid.template.jinja2",
        "DIRS": [ os.path.join(BASE_DIR, "templates"), os.path.join(BASE_DIR, "views") ],
        "CSRF": "bermoid.security.csrf.CSRF",
        "OPTIONS": {
            "autoscape": True,
            "context_processors": [
                "bermoid.template.context_processors.URLContextProcessor",
                "bermoid.template.context_processors.CSRFContextView"
            ],
            "extensions": [
                "bermoid.template.extensions.URLConstructor"
            ]
        }
    }
]
...
CSRF = [
    {
        "BACKEND": "bermoid.security.csrf.CSRF",
        "OPTIONS": {
...
# MIDDLEWARE = [
#    "bermoid.middlewares.XFrameOptionsMiddleware", # String middleware with default configuration.
#    {
#        "origin": "bermoid.middlewares.GzipMiddleware",
#        "order": 1,
...
# For additional details, please visit: http://bermoid.vvfin.in/middlewares/core
...
MIDDLEWARES = [
    "bermoid.middlewares.XFrameOptionsMiddleware",
    "bermoid.middlewares.GzipMiddleware",
    "bermoid.middlewares.StaticMiddleware",
    "bermoid.middlewares.TrustedhostMiddleware",
    "bermoid.middlewares.CSRFMiddleware",
    "bermoid.middlewares.MediaMiddleware"
]
...
# For more info visit :: http://bermoid.vvfin.in/middlewares/hooks

STAGE_HANDLERS = [
    { "origin": "bermoid.core.backend.sessions.InMemorySessionBeforeStage", "stage": "before"}
]

### Sessions Configuration....

# SESSIONS_CONFIG is a list of dictionaries containing session configuration settings for bermoid.
...
# For further details on session configurations, refer to bermoid documentation.
...
STORAGE_BACKEND = [
    {
        "static": [
            "bermoid.core.files.static.storage"
        ]
    }
]
